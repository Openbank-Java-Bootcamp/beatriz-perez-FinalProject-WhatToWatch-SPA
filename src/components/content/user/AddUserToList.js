...
// Styles:
import styles from "./AddUserToList.module.scss";

function AddUserToList({ participant }) {
    ...
    const addParticipantToList = (listId) => {
        const requestBody = { id: participant.id, };
        const storedToken = localStorage.getItem("authToken");
        axios
            .patch(`${API_URL}/api/lists/participant/${listId}`, requestBody, {
            headers: { Authorization: `Bearer ${storedToken}` },
            })
            .then((response) => {
                setBoxMessage("Participant added!")
    ...
    // Add participant to a new list
    const  adParticipantToNewList = (e) => {
        e.preventDefault();
        ...
        axios
            ...
            .then((response) => {
                ...
                addParticipantToList(response.data.id);
            })
            .catch((error) => console.log(error));     
    ...
    return (
        <>
            <button className={styles.button} onClick={getUserLists}>Invite to watchlist</button>
            {isAdding && (
                        ...
                        <div className={styles.addBox__column} >
                            ...
                            {userLists.map(list => 
                                <div className={styles.addBox__listBox}>
                                    ...
                                    <button onClick={() => {addParticipantToList(list.id)}} className={[styles.button, styles.buttonSmall].join(" ")} type="button">Add</button>
                                </div>
                            )}
                        </div>
                        <div className={styles.addBox__column} >
                            <h2 className={styles.title} >Create a new WatchList</h2>
                            <form className={styles.form} onSubmit={adParticipantToNewList}>
                                <input 
